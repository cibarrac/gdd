...
{
    public function checkLogin($username, $mypassword)
    {
        $conf = new GlobalConf;
        ...
        $stmt = $db->conn->prepare("SELECT * FROM ".$tbl_members." WHERE email = :username");
        ...
        if ($curr_attempts >= $max_attempts && $timeDiff < $login_timeout) {
            ...
            if (password_verify($mypassword, $result['password']) && $result['verified'] == '1') {
              ...
              if (isset($query_data)) {
                $stmtData = $db->conn->prepare($query_data);
                  $stmtData->bindParam(':idrole', $iduser);

                  $res= $stmtData->execute();

                  $result = $stmtData->fetch(PDO::FETCH_ASSOC);

                    // Con esto no necesitan consultar a la base de datos frecuentemente para sacar esa informacion. eso es lo que esta corriendo ahora?
                    // mira, otra cosa, al momento de obtener el nombre del profesor, aparecen caracteres de mas. MongoInsertBatch
                    // donde esta eso en codigo
                  $_SESSION['data-user'] = [
                      'username' => $myusername,
                      'role' => $role,
                      ...
                      'extra' => null // En caso de requerir más datos para la sesion deberán colocarse aqui
                  ];
              } else {
                  $_SESSION['data-user'] = [
                      'username' => $myusername,
                ...
                $success = "<div class=\"alert alert-danger alert-dismissable\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button>Su cuenta fue creada pero no  verficada. Contacte con el administrador</div>";

            } else {

                //Wrong username or password
                $success = "<div class=\"alert alert-danger alert-dismissable\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button> Usuario/Contraseña Erroneos </div>";
