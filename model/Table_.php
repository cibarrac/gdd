 ...
 <?php
  function evaluate_cursos($table,$NumeroCurso)
  {
     ...
     <button  type="checkbox" class="btn btn-warning fa fa-address-book-o" onclick="inscribir(<?php echo $NumeroCurso;?>,<?php echo $NumeroCurso;?>);">
     </button>
 ...
 function createTable($view){
    ...
    if(mysql_num_rows($list)>0){
             while($row = mysql_fetch_array($list)){   ?>
                   <tr <?php
                   if(isset($row['sign1'])){
                        if($row['sign1']==1 && $row['sign2']==1){ echo "class='success'";} else {echo "class='warning'";}
                   }
